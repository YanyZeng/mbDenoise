...
for(k in 1:iter){
  print(k)
  set.seed(k)
  f1 <- matrix(0,nrow = n.n*0.4, ncol = n.factors)
  for(i in 1:(n.n*0.4)){
    f1[i,] <- MASS::mvrnorm(1, mu=c(-1,1), Sigma=diag(0.5,n.factors,n.factors))
  }
  f2 <- matrix(0,nrow = n.n*0.3, ncol = n.factors)
  for(i in 1:(n.n*0.3)){
    f2[i,] <- MASS::mvrnorm(1, mu=c(2,1.5), Sigma=diag(0.5,n.factors,n.factors))
  }
  f3 <- matrix(0,nrow = n.n*0.3, ncol = n.factors)
  for(i in 1:(n.n*0.3)){
    f3[i,] <- MASS::mvrnorm(1, mu=c(0.5,-1.5), Sigma=diag(0.5,n.factors,n.factors))
  }
  f <- rbind(f1,f2,f3)
  betaj <- matrix(0,nrow = n.w, ncol = n.factors)
  for(j in 1:n.w){
    betaj[j,] <- runif(n.factors, -2, 2)
  }
  beta0 <- runif(n.w,-1,1)
  alpha0 <- runif(n.w,-1,1)
  
  l <- matrix(beta0,n.n,n.w,byrow=TRUE) +matrix(alpha0,n.n,n.w) + f %*% t(betaj)
  
  lambda <- exp(l)
  X <- matrix(0,n.n,n.w,byrow = TRUE)
  for(i in 1:n.n){
    for(j in 1:n.w){
      X[i,j] <- rnbinom(n=1,size=1,mu=lambda[i,j])
    }
  }
  X_cov<- c(rep(1,n.n/2),rep(0,n.n/2))
  X_ori <- exp(f %*% t(betaj)+matrix(beta0,n.n,n.w,byrow=TRUE))
  gamma <- rep(0,n.w)
...
save.image("simulation/sim_gllvm1.RData")
